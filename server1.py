...
Scrapes all 4 stores in parallel using Playwright + asyncio.

Render build command:  pip install -r requirements.txt && playwright install chromium
Render start command:  playwright install chromium && gunicorn server:app --timeout 120
"""
...
async def scrape_one(browser, store, query):
    """Scrape a single store using an existing browser instance."""
    print(f"  [{store['label']}] opening page...")
    results = []
    try:
        ...
        try:
            ...
            print(f"    [{store['label']}] no tiles found")
            await page.close()
            return []

        tiles = await page.query_selector_all(store["tile"])
        print(f"    [{store['label']}] {len(tiles)} tiles")
    ...
    print(f"    [{store['label']}] → {len(results)} prices")
    return results


async def scrape_all(query):
    """Launch one browser, scrape all stores in parallel."""
    async with async_playwright() as p:
        browser = await p.chromium.launch(
            headless=True,
            args=["--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage"]
        )
        tasks = [scrape_one(browser, store, query) for store in STORES]
        results_list = await asyncio.gather(*tasks)
        await browser.close()

    return {store["key"]: results for store, results in zip(STORES, results_list)}
...
def run_search(query):
    ...
    try:
        return loop.run_until_complete(scrape_all(query))
    finally:
        loop.close()
