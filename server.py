...
Calls store internal JSON APIs directly. No browser/scraping needed.

Render build command:  pip install -r requirements.txt
Render start command:  gunicorn server:app --timeout 60
"""
...
def clean_price(val):
    ...
    if isinstance(val, (int, float)):
        p = float(val)
        return round(p, 2) if 0.50 <= p <= 120 else None
    if isinstance(val, str):
        m = re.search(r"(\d{1,3}(?:\.\d{2})?)", val.replace(",", ""))
        if m:
            p = float(m.group(1))
            return round(p, 2) if 0.50 <= p <= 120 else None
    return None
...
def search_coles(query):
    url = f"https://www.coles.com.au/api/2.0/page/search?q={urllib.parse.quote(query)}&pageSize=5&pageNumber=1"
    try:
        data = fetch_json(url, {"Referer": "https://www.coles.com.au/", "Origin": "https://www.coles.com.au"})
        results = []
        products = (data.get("searchResults") or {}).get("results") or []
        for p in products:
            name  = p.get("name") or p.get("fullName") or ""
            price = clean_price(p.get("pricing", {}).get("now") or p.get("price"))
            if name and price and len(results) < 2:
                results.append({"name": name[:65], "price": price})
        print(f"  [Coles] {len(results)} results")
        return results
...
def search_woolworths(query):
    url = (
        ...
        f"?searchTerm={urllib.parse.quote(query)}&pageNumber=1&pageSize=5"
        f"&sortType=TraderRelevance&isSpecial=false"
    )
    try:
        data = fetch_json(url, {
        ...
        for p in products:
            ...
            price = clean_price(p.get("Price") or p.get("WasPrice"))
            if name and price and len(results) < 2:
                results.append({"name": name[:65], "price": price})
        print(f"  [Woolworths] {len(results)} results")
        return results
    except Exception as e:
...
# ── Aldi (HTML parse — no JSON API available) ──────────────────────────────────

def search_aldi(query):
    url = f"https://www.aldi.com.au/search/?q={urllib.parse.quote(query)}&lang=en_AU"
    try:
        req = urllib.request.Request(url, headers={**HEADERS,
        ...
        results = []
        # Aldi embeds product JSON in the page as data attributes or JSON blocks
        # Try structured data first
        for m in re.finditer(r'"name"\s*:\s*"([^"]{5,80})"\s*,\s*(?:[^}]*?"price"\s*:\s*"?([\d.]+)"?)', html):
            price = clean_price(m.group(2))
            if price and len(results) < 2:
                results.append({"name": m.group(1)[:65], "price": price})

        if not results:
            # Fallback: regex over plain text
            text = re.sub(r"<[^>]+>", " ", html)
            for m in re.finditer(r"([A-Za-z][^\$\n]{5,60}?)\s*\$\s*(\d{1,3}(?:\.\d{2})?)", text):
                price = clean_price(m.group(2))
                if price and len(results) < 2:
                    results.append({"name": m.group(1).strip()[:65], "price": price})

        print(f"  [Aldi] {len(results)} results")
        return results
    except Exception as e:
...
def search_foodland(query):
    url = f"https://shop.foodland.com.au/search?q={urllib.parse.quote(query)}"
    try:
        req = urllib.request.Request(url, headers={**HEADERS,
            "Referer": "https://shop.foodland.com.au/",
            "Origin":  "https://shop.foodland.com.au",
            "Accept":  "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
        })
        with urllib.request.urlopen(req, timeout=12) as r:
            html = r.read().decode("utf-8", errors="replace")
        ...
        print(f"  [Foodland] {len(results)} results")
        return results
    except Exception as e:
        print(f"  [Foodland] ERROR: {e}")
        return []
...
def search():
    ...
    # Run all stores (sequentially — simple and reliable on free tier)
    results = {
        "coles":    search_coles(item),
        "woolies":  search_woolworths(item),
        "aldi":     search_aldi(item),
        "foodland": search_foodland(item),
    }
...
function renderResults(query,results){
  ...
  document.getElementById('results').innerHTML=`<div class="res-hdr"><div class="res-title">Price Comparison</div><div class="res-query">"${query}"</div></div><div class="store-grid">${cards}</div>${summary}<div class="disc">⚡ Prices from store APIs. Always confirm on the retailer's website before shopping.</div>`;
}
